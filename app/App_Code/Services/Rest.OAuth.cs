        ...
        public virtual void ExecuteOAuthGetServices(JObject schema, JObject payload, JObject result)
        {
            ...
            foreach (var appName in sortedApps.Keys)
            {
                ...
                if (redirectUri.EndsWith(AppIdentityRedirectPath))
                {
                    ...
                    var link = AddLink(serviceName, "GET", links, new Uri(new Uri(redirectUri.Substring(0, (redirectUri.Length - AppIdentityRedirectPath.Length))), "/v2").ToString());
                    ...
                    if (includeLocal)
                    {
                        ...
                        if (!string.IsNullOrEmpty(localRedirectUri))
                            AddLink((serviceName + "-local"), "GET", links, new Uri(new Uri(localRedirectUri.Substring(0, (localRedirectUri.Length - AppIdentityRedirectPath.Length))), "/v2").ToString());
                    }
                }
            }
            result["count"] = count;
