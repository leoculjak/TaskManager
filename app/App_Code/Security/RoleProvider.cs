...
namespace MyCompany.Security
{
    ...
    public class ApplicationRoleProviderBase : RoleProvider
    {
        ...
        static ApplicationRoleProviderBase()
        {
            ...
            Statements[RoleProviderSqlStatement.CreateRole] = "insert into Roles (Name) values (@RoleName)";
            Statements[RoleProviderSqlStatement.DeleteRole] = "delete from Roles where Name = @RoleName";
            Statements[RoleProviderSqlStatement.DeleteRoleUsers] = "delete from UserRoles where RoleID in (select Id from Roles where Name = @RoleName)";
            Statements[RoleProviderSqlStatement.GetAllRoles] = "select Name RoleName from Roles";
            Statements[RoleProviderSqlStatement.GetRolesForUser] = "\r\nselect Roles.Name RoleName from Roles Roles \r\ninner join UserRoles UserRoles on Roles.Id = UserRoles.RoleID \r\ninner join Users Users on Users.Id = UserRoles.UserID\r\nwhere Users.UserName = @UserName";
            Statements[RoleProviderSqlStatement.GetUsersInRole] = "select UserName UserName from Users where Id in (select UserID from UserRoles where RoleID in (select Id from Roles where Name = @RoleName))";
            Statements[RoleProviderSqlStatement.IsUserInRole] = "\r\nselect count(*) from UserRoles\r\nwhere\r\n    UserID in (select Id from Users where UserName = @UserName) and \r\n    RoleID in (select Id from Roles where Name = @RoleName)";
            Statements[RoleProviderSqlStatement.RemoveUserFromRole] = "\r\ndelete from UserRoles\r\nwhere\r\n   UserID in (select Id from Users where UserName = @UserName) and\r\n   RoleID in (select Id from Roles where Name = @RoleName)";
            Statements[RoleProviderSqlStatement.RoleExists] = "select count(*) from Roles where Name = @RoleName";
